...
function test() {
  $("body").empty();
  GlobalStore.Symstore.tagText(GlobalStore.TextStore.tokenizedText, "mouseenter", function(event) {
    let parentSpan = $(this);
    ...
    if (event.type === "mouseenter") {
      ...
      if (!$(this).children(".wordtooltip").length) {
        ...
        let callback = function(event) {
          ...
          GlobalStore.TextStore.setWord(id, synonym);
        ...
        let synonyms = GlobalStore.Symstore.getSynonyms(word, 3);
    ...
    // GlobalStore.TextStore.setWord(intId, "TEST")
  });
}
function testanalyze() {
  GlobalStore.Symstore.analyzeText(GlobalStore.TextStore.text);
}

$(document).ready(function () {
  // Initialize the store and keep track of it (or remove to reset)
  GlobalStore = Database.initialize();
  /* GlobalStore.addStore("WordTest", new WordStore())
  GlobalStore.WordTest.setWord(1, "test");
  GlobalStore.WordTest.setWord(1, "test2");
  GlobalStore.WordTest.setWord(1, "test3");
  GlobalStore.WordTest.setWord(2, "Tjo") */

  // SHOULD NEVER REALLY BE USED this is due to orphaned snapshot
  // GlobalStore.removeStore("WordTest");
  ...
  /* GlobalStore.Symstore.tagText(GlobalStore.TextStore.tokenizedText, function(){
  ...
  console.log(GlobalStore.TextStore.tokenizedText);

  console.log("HTML:", GlobalStore);
});
